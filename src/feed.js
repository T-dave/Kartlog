	...
	return(
		 	...
		 	
      {
      	feed.map((itemm, index)=>{
      		return(
      		<ScrollView horizontal={true} key={index} style={{flexDirection:'row', flex:1}} showsHorizontalScrollIndicator={false}>
      			{
      				feedPic.map((item, index)=>{
      					return(
      					<FeedImg source={item.image} key={item.key}>
